...
GPU_COMMON="$REPO_ROOT/ui/fb-shm-gpu-common.c"
...
test_hello_rejects_unsafe_or_competing_clients() {
    ...
    require_text "static GHashTable *fb_shm_d3d_console_owners" "$FB_SHM"
    require_text "g_hash_table_lookup(fb_shm_d3d_console_owners, d->con)" "$FB_SHM"
    require_text "g_hash_table_insert(fb_shm_d3d_console_owners, d->con, d)" "$FB_SHM"
    require_function_text "$FB_SHM" "fb_shm_handle_hello" \
...
test_handoff_uses_persistent_bh_after_dcl_work() {
    ...
    require_text "QEMUBH *d3d_handoff_bh" "$FB_SHM"
    require_text "qemu_bh_new(fb_shm_d3d_handoff_bh, d)" "$FB_SHM"
    require_text "qemu_bh_schedule(d->d3d_handoff_bh)" "$FB_SHM"
    require_text "qemu_bh_cancel(d->d3d_handoff_bh)" "$FB_SHM"
    ...
    require_text ".dpy_gl_update          = fb_shm_gl_update" "$FB_SHM"
    require_function_text "$FB_SHM" "fb_shm_gl_update" "fb_shm_commit_gl_frame(d)"
    require_function_order "$FB_SHM" "fb_shm_broadcast_texture_dmabuf_frame" \
        "d->d3d_handoff_layout = layout" \
        "d->d3d_handoff_scheduled = true" \
        "qemu_bh_schedule(d->d3d_handoff_bh)"
    require_count "fb_shm_gpu_backend_d3d_release0(d->gl_gpu_backend)" 1 "$FB_SHM"
    require_function_text "$FB_SHM" "fb_shm_d3d_handoff_bh" \
    ...
    require_function_order "$FB_SHM" "fb_shm_commit_gl_frame" \
        "fb_shm_gl_pbo_issue" \
...
test_release_pending_and_send_order_is_atomic() {
    ...
    require_function_order "$FB_SHM" "fb_shm_d3d_handoff_bh" \
...
test_done_requires_exact_sequence_and_nonblocking_acquire() {
    ...
    require_function_text "$FB_SHM" "fb_shm_handle_gpu_frame_done" \
        "uint64_t sequence = ((uint64_t)req->h << 32) | req->w"
    require_function_order "$FB_SHM" "fb_shm_handle_gpu_frame_done" \
    ...
    require_text "case FB_SHM_CTL_GPU_FRAME_DONE:" "$FB_SHM"
...
test_pending_frame_stops_all_texture_access() {
    ...
    require_function_text "$FB_SHM" "fb_shm_commit_gl_frame" \
...
test_disconnect_reclaims_with_timer_without_unsafe_unblock() {
    ...
    require_function_order "$FB_SHM" "fb_shm_d3d_cancel_pending" \
        "fb_shm_gpu_backend_d3d_acquire0(d->gl_gpu_backend)" \
        ...
        "fb_shm_d3d_set_gl_blocked(d, false)"
    require_function_order "$FB_SHM" "fb_shm_client_drop" \
        "c->owner->d3d_sync_client = NULL" \
        "fb_shm_d3d_cancel_pending(c->owner)"
    reject_function_text "$FB_SHM" "fb_shm_client_drop" \
        "fb_shm_gpu_backend_reset"
    reject_function_text "$FB_SHM" "fb_shm_client_drop" \
        "fb_shm_d3d_set_gl_blocked"
    require_function_text "$FB_SHM" "fb_shm_d3d_reclaim_timer_cb" \
...
test_object_delete_waits_for_safe_reclaim() {
    ...
    require_function_order "$FB_SHM" "fb_shm_export_can_be_deleted" \
        ...
        "return false;" \
        "return true;"
    require_text "ucc->can_be_deleted = fb_shm_export_can_be_deleted" "$FB_SHM"
}
...
test_sources_do_not_use_unwrap() {
    ...
    for file in "$FB_SHM" "$GPU_BACKEND" "$GPU_COMMON" "$GPU_HEADER" \
