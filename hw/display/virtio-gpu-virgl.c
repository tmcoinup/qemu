                                  ...
                                  struct virtio_gpu_ctrl_command *cmd)
{
    ...
    if (ss.resource_id && ss.r.width && ss.r.height) {
        ...
        if (ext.has_dmabuf_export && ext.planes == 1 &&
            info.fd >= 0 && info.drm_fourcc) {
            /*
             ...
             * dmabuf 显示路径。当前 sideband ABI 只能表达单平面，所以
             * ext.planes != 1 时必须回退，不能丢失 QEMU 11 的平面元数据。
             */
