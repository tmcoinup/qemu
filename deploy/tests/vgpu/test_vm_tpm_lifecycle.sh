...
[[ "${VM_TPM_QEMU_ARGS[1]}" == \
    "socket,id=chrtpm,path=$VM_ROOT/instances/vm41/run/swtpm.sock" ]] \
...
[[ "${VM_TPM_QEMU_ARGS[1]}" == \
    "socket,id=chrtpm,path=$VM_ROOT/instances/vm40/run/swtpm.sock" ]] \
...
if (
    ...
    unset VM_ASSET_DIR VM_DISK_ARCHIVE_DIR VM_BASE_ARCHIVE_DIR
    unset VM_NVRAM_BACKUP_DIR
    vm_storage_init
    vm_tpm_plan 45
...
assert_not_exists "$VM_ROOT/instances/vm42" 'failed capability instance'
VM_TPM_SWTPM_BIN=$FAKE_SWTPM
...
assert_not_exists "$VM_ROOT/instances/vm43" 'missing localca instance'
