...
16667us R535 REGION 周期和 `host` 光标。默认始终保留宿主即时箭头，以跟手为
优先；Windows 拖窗口时 primary framebuffer 的延迟箭头可能形成可接受的重影。
...
auto 不把“有 damage”或“附近有黑白像素”当成光标。QEMU 从启动器提供的
`aero_arrow.cur` 选取 32×32 帧，抽取高不透明度深色边缘和完全不透明浅色内部两组
样本；只检查 500 ms 内记录的左键拖动坐标及其 ±2 px 邻域。深色和浅色样本必须
