...
#
# 安装目标：/usr/local/libexec/qemu-g11-vlan-down（root:root 0755）。
...
readonly HELPER="/usr/local/libexec/qemu-g11-vlan-tap"
...
log_cleanup_failure() {
    ...
    logger -t qemu-g11-vlan-down -- "$message" 2>/dev/null || true
}
...
main() {
    ...
    if ! [[ "$tap" =~ ^g11t[0-9]{1,10}$ ]]; then
        printf 'ERROR: 拒绝清理非 g11t<实例号> 接口: %s\n' "$tap" >&2
        return 2
    fi
