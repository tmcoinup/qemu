...
#
# 跑完后 delete-vm.sh + deploy/scripts/start-vm.sh 都从这个干净 baseline 复制起，
# 不再需要 setup-guest 装 driver/license。
#
# 用法:
#   ./deploy/scripts/promote-base.sh           # vm1
#   ./deploy/scripts/promote-base.sh <vm_id>
#   ./deploy/scripts/promote-base.sh -y        # 跳过确认
#
set -euo pipefail
...
echo "  ./deploy/scripts/delete-vm.sh ${VM_ID} -y  # 删 vm${VM_ID} (不动 base)"
echo "  ./deploy/scripts/start-vm.sh ${VM_ID}          # 缺盘时 create-disk 会从 bases/ 自动复制"
