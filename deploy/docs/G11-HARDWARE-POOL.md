...
| 类别 | 当前结果 |
|---|---|
| 正常新建 CPU | 2 款：Intel Core i7-4820K、Core i7-3820；家用、4C/8T、无核显 |
| 正常新建主板 | 3 款/3 品牌：ASUS P9X79、Gigabyte GA-X79-UP4、ASRock X79 Extreme4 |
| 正常内存档 | 4G、8G、12G、16G；DDR3-1600/1866，Kingston/Samsung/Elpida/Micron |
| 通道 | 4G/8G 为两根双通道；12G 为 3×4G 三通道；16G 为 4×4G 四通道 |
| 正常整机组合 | 48 条，全部是 X79/LGA2011 原子白名单 |
| 完整目录 | 10 CPU、16 主板、37 内存、312 整机；其中 261 archived、3 legacy compatibility |
...
```text
cpu=10 board=16 memory=37 combination=312
new_default=48 explicit_new=0 archived=261 legacy=3
brands board=3 memory=4 ssd=5
```
...
因此没有用 Xeon、6 核或带核显的 CPU 凑第三款。默认性能优先级先选
`i7-4820K + DDR3-1866`，再选其它 i7-4820K，最后才是 i7-3820。宿主不支持某个
模型时仍由 realization 门禁跳过，不会只看名字强启。
...
DDR3-1866 使用 Intel Ivy Bridge-E 非 ECC 验证表中的 Elpida
`EBJ40UG8BFW0-JS-F` 与 Micron `MT8KTF51264AZ-1G9`；只和 i7-4820K 及支持 1866
的 P9X79/GA-X79-UP4 组合。ASRock X79 Extreme4 与 i7-3820 按较低共同上限使用
DDR3-1600 Kingston/Samsung。4G/8G 仍有审核的对应双条组合，因此四个容量档都能
覆盖三块主板和两款 CPU；不会为了追求频率违反板/CPU/模组上限。
...
三款 NVMe 都声明 Gen3 x4，只有 `i7-4820K + X79 被动转接器` 能满足自动合同。
