...
这份电源基线是该问题**唯一**的解法。QEMU 侧曾有一个 `keepalive` 选项，在画面静止约
10 秒后向 guest 注入一次 Scroll Lock 充当兜底；它已被移除，因为那个前提是错的：
Scroll Lock 是 DNF 的内置录屏热键，而且是切换键，注入奇数次就把游戏留在正在录屏的
状态。任何一个键都无法保证在任意全屏程序里无副作用，所以也没有换用别的键。fb-shm
现在只在画面静止时打日志（`fb-shm: id=... guest display idle for Ns`），不再注入任何输入。

因此**必须**确认本节的电源基线已经生效：它失效时不再有任何兜底。
