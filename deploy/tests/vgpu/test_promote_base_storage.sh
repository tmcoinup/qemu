...
default_before=$(sha256sum "$VM_ROOT/shared/bases/win10-base.qcow2" | awk '{print $1}')
...
[[ "$(sha256sum "$VM_ROOT/shared/bases/win10-base.qcow2" | awk '{print $1}')" == \
...
first_base_sha=$(sha256sum "$VM_ROOT/shared/bases/win10-base.qcow2" | awk '{print $1}')
...
printf 'old-portable-proof\n' \
    >"$VM_ROOT/shared/bases/win10-base.qcow2.vgpu-portable.json"
printf 'third-generation-source\n' >"$VM_ROOT/$TEST_VM_ID/disk.qcow2"
run_seal "$TEST_VM_ID" -y >"$TMP_DIR/sidecar-archive.out"
[[ ! -e "$VM_ROOT/shared/bases/win10-base.qcow2.vgpu-portable.json" ]] \
    || fail "newly sealed base retained the old portable sidecar"
sidecar_archive=$(find "$VM_ROOT/shared/bases/archive" -type f \
...
[[ "$(stat -c %i "$PROTOCOL_VM_ROOT/shared/bases/win10-base.qcow2")" == \
...
[[ "$(stat -c %i "$CHAIN_VM_ROOT/shared/bases/win10-base.qcow2")" == \
...
if find "$VM_ROOT/shared/bases" -maxdepth 1 -name '*.partial.*' -print | grep -q .; then
