...
克隆首次启动时只运行一次。随后自动安装该 VM 专属的 x86/x64 系统 NVAPI，并请求
重启一次；宿主会把这次重启自动转换为一个全新的 QEMU 进程冷启动，避免固件热重启
卡死。冷启动后的同一条链还会自动应用 Guest Lite 2.6.7 克隆快速路径，关闭 Defender 实时扫描、三种
...
- 基础镜像内预置的是授权 `VgpuPortable.exe`、首启脚本和 OOBE 应答文件；
  ...
  光盘挂入，来宾复制到受保护的 `ProgramData` 后完成安装和冷启动验证。
...
自动首启成功时不要求按键：Windows 会请求重启一次，启动器在确认初始化光盘已安全
弹出且 QMP 原因确为 `guest-reset` 后，自动关闭旧 QEMU 并只冷启动一次；验收后自动
完整关机。第一次自动冷启动
...
此命令返回后不要立即再次启动或强制停止 VM9。等待启动器自动完成一次 QEMU 冷启动，
随后让新 QEMU 因来宾完整关机自行退出。命令行环境再执行下面两条；图形界面中等价操作
是点一次“初始”，成功后再点“启动”：
...
弹出介质，宿主立即热拔整个 `scsi-cd + usb-bot`；随后只有同一条已认证链路的
`guest-reset` 能触发一次 QEMU 冷启动，以后普通启动都不再显示光驱。这个临时设备
使用审核过的真机型号且没有虚构序列号，不需要手工卸载或重启宿主。
...
6. 不需要操作 OOBE，也不要双击 EXE。克隆会自动完成：
   ...
   - 安装 x86/x64 系统 NVAPI 与显示器身份，并由宿主自动完成一次 QEMU 冷启动；
...
只经历一次自动首启、一次宿主自动 QEMU 冷启动、一次最终完整关机和一次“初始”，不需要
用户额外运行软件、关机或刷新。
...
- 若报告显示 `0x80073cf2` 以及某个包“installed for a user, but not provisioned for
  ...
  的脚本。
- 自动首启成功不等待按键：5 秒后 Windows 请求重启，宿主自动换成全新 QEMU 进程
  冷启动一次，最终验收后自动关机；不需要重启宿主。失败时窗口会
  保留红色错误并停在“请按任意键继续”，同时写入下面的完整错误文件。
- 第一次自动冷启动后即使已经进入桌面，也必须等“克隆初始化仍在继续”状态窗切换为
