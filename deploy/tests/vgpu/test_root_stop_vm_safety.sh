...
VM_ROOT="$TMP_DIR" "$STOP_VM" "$VM_ID" >"$TMP_DIR/cleanup.out"
...
mkdir -p "$TMP_DIR/instances/vm${CANONICAL_ID}/run" \
    "$TMP_DIR/instances/vm${CANONICAL_ID}/tpm/state"
printf '%s\n' '00000000-0000-0000-0000-999999999998' \
    >"$TMP_DIR/instances/vm${CANONICAL_ID}/run/mdev.uuid"
touch "$TMP_DIR/instances/vm${CANONICAL_ID}/run/qemu.pid" \
    "$TMP_DIR/instances/vm${CANONICAL_ID}/run/qmp.sock" \
    "$TMP_DIR/instances/vm${CANONICAL_ID}/run/monitor.sock"
printf '%s\n' 999999999 >"$TMP_DIR/instances/vm${CANONICAL_ID}/run/swtpm.pid"
touch "$TMP_DIR/instances/vm${CANONICAL_ID}/tpm/state/tpm2-00.permall"
python3 - "$TMP_DIR/instances/vm${CANONICAL_ID}/run/swtpm.sock" <<'PY'
import socket
import sys
...
[[ -z "$(find "$TMP_DIR/instances/vm${CANONICAL_ID}/run" -mindepth 1 -print -quit)" ]] \
    || fail "canonical per-VM runtime files were not removed"
[[ -e "$TMP_DIR/instances/vm${CANONICAL_ID}/tpm/state/tpm2-00.permall" ]] \
