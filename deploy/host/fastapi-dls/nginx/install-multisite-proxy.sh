#!/usr/bin/env bash
# Create a host-Nginx 80/443 vhost following the established Cloudflare multi-site layout.

set -euo pipefail
umask 077

SCRIPT_DIR=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)
TEMPLATE="$SCRIPT_DIR/multisite-vhost.conf.template"
NGINX_CONF_DIR=${NGINX_CONF_DIR:-/etc/nginx/conf.d}
DLS_UPSTREAM_CERT=${DLS_UPSTREAM_CERT:-"$SCRIPT_DIR/../state/cert/webserver.crt"}

log() {
    printf '[fastapi-dls nginx] %s\n' "$*"
}

die() {
    printf '[fastapi-dls nginx] ERROR: %s\n' "$*" >&2
    exit 1
}
...
usage() {
    cat <<'EOF'
Usage:
  sudo ./nginx/install-multisite-proxy.sh install <domain> <upstream-port> <frontend-cert> <frontend-key>
  sudo ./nginx/install-multisite-proxy.sh status [upstream-port]

Examples:
  sudo ./nginx/install-multisite-proxy.sh install dls.example.com 9443 \
    /etc/nginx/ssl/example/dls.origin.pem /etc/nginx/ssl/example/dls.origin.key
  sudo ./nginx/install-multisite-proxy.sh status 9443

install creates a standalone 80/443 Nginx virtual host; Docker never owns public
port 443. The domain must be Cloudflare orange-cloud proxied to this host and the
host must define $origin_from_cloudflare (for example with 00-cloudflare-realip.conf).
The frontend certificate must be a Cloudflare Origin CA certificate or a public CA
certificate obtained with DNS-01; HTTP-01 is intentionally not used here.
EOF
}
...
install_proxy() {
    ...
    [[ -s "$DLS_UPSTREAM_CERT" ]] || \
        die "找不到 DLS 上游证书: $DLS_UPSTREAM_CERT；先运行 ./dlsctl.sh configure $domain 443 $upstream_port"
    certificate_covers_domain "$frontend_cert" "$domain" || \
        die "前端证书 SAN 未覆盖 $domain: $frontend_cert"
    certificate_covers_domain "$DLS_UPSTREAM_CERT" "$domain" || \
        die "DLS 上游证书 SAN 未覆盖 $domain；重新运行 ./dlsctl.sh configure $domain 443 $upstream_port"
    require_cloudflare_guard
    ...
    log "多站点反代已启用: https://$domain:443 -> https://127.0.0.1:$upstream_port"
    log 'Cloudflare 源站限制不等于 guest 网络白名单；生产使用前仍应在防火墙或 Cloudflare WAF 限制受控 guest/VPN 网段。'
}

status_proxy() {
    local upstream_port=${1:-9443}
    validate_port "$upstream_port"
    need_command nginx
    need_command curl
    nginx -t
    curl --insecure --fail --silent --show-error "https://127.0.0.1:${upstream_port}/-/health" >/dev/null
    log "本机 DLS 上游健康: https://127.0.0.1:${upstream_port}/-/health"
}
