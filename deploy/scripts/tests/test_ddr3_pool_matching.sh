...
assert_smbios_memory_shape() {
    local socket="$1" expected_type="$2" expected_voltage="$3"
    local type17_args

    CPU_SOCKET="$socket"
    type17_args="$(stealth_smbios_args | grep '^type=17,')"
    [[ ",$type17_args," == *",memtype=${expected_type},"* ]] \
        || fail "socket=$socket 的 SMBIOS 内存类型错误: $type17_args"
    [[ ",$type17_args," == *",typedetail=0x80,"* ]] \
        || fail "socket=$socket 未报告同步内存: $type17_args"
    [[ ",$type17_args," == *",width=64,totalwidth=64,rank=1,"* ]] \
        || fail "socket=$socket 的内存位宽/Rank 错误: $type17_args"
    [[ ",$type17_args," == *",voltage=${expected_voltage},"* ]] \
        || fail "socket=$socket 的内存电压错误: $type17_args"
}

stealth_pick_profile
assert_smbios_memory_shape AM3 0x18 1500
assert_smbios_memory_shape AM4 0x1A 1200
