...
GNOME/Ubuntu 桌面下，默认启动会启用 viewer 侧的动态宿主快捷键保护：viewer 有键盘焦点或鼠标在 VM 窗口内时临时关闭宿主侧 `Super`/`Meta`、`Alt+Tab`、锁屏等 GNOME/IBus 快捷键，失焦且鼠标移出、最小化、隐藏或退出立即恢复。保留宿主快捷键行为可加 `--no-tame-gnome`。
