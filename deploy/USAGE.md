...
GNOME/Ubuntu 桌面下，`start-vm.sh` 会启用 viewer 侧的动态宿主快捷键保护：鼠标在 VM 窗口内时，临时关闭宿主侧 `Super`/`Meta`、`Alt+Tab`、锁屏等 GNOME/IBus 快捷键；鼠标移出、最小化、隐藏或退出都会立即恢复宿主按键。需要完全保留宿主快捷键行为时加 `--no-tame-gnome`。
