                                 ...
                                 bool owns_fd)
{
    ...
    /*
     * 中文注释：这里不复制像素，只把一个现有 dma-buf fd 适配到 QEMU 11
     * 的多平面 QemuDmaBuf 接口。owns_fd=true 时本包装层负责最终关闭 fd；
     * 否则 fd 仍归 resource/virglrenderer 所有，跨进程消费者必须复制 fd。
     */
    dmabuf->buf = qemu_dmabuf_new(width, height, &offset, &stride,
                                  x, y, backing_width, backing_height,
                                  fourcc, modifier, &dmabuf_fd,
                                  1, true, y0_top);
                         ...
                         struct virtio_gpu_rect *r)
{
    return virtio_gpu_create_dmabuf_from_fd(
        ...
        qemu_pixman_to_drm_format(fb->format), DRM_FORMAT_MOD_INVALID,
        false, false);
}
