...
- 将**所有**电源计划的显示器超时和系统空闲睡眠（交流/电池）设为“从不”，
  防止无人操作后 guest 自己黑屏；只设当前计划不够——Windows 更新、驱动安装或
  厂商工具把计划切回“平衡”后黑屏就会重新出现，而画面冻结时主机端仍按原帧率
  推流，消费端看到的是「帧率正常、画面不动」。计划由 `powercfg /list`
  动态枚举，不依赖本地化名称、固定数量或 VM1 GUID；没有显式注册表覆盖的计划会
  按继承值采样，回滚时删除工具新增覆盖来恢复继承；
