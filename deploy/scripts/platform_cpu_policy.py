...
# Intel ARK、Intel ordering、Intel CPUID guidance 与 ASUS CPU support 表共同
# 固化这些值。映射刻意包含所有 H310 supported SKU，新增型号必须先扩展审计表。
...
def platform_identity_id(cpu: dict[str, Any], board: dict[str, Any]) -> str:
    """由 CPU 系列、socket 和 ASUS 主板型号生成稳定目录 ID。"""
    name = cpu["name"]
    ...
    if board["manufacturer"] != "ASUSTeK COMPUTER INC.":
        raise ValueError(
            f"当前已审计平台 ID 只接受 ASUSTeK 主板：{board['manufacturer']}"
        )
    product = re.sub(r"\.0\b", "", board["product"]).replace(".", "")
    board_token = re.sub(r"[^a-z0-9]+", "-", product.lower()).strip("-")
    return "-".join((
        vendor_token, cpu["socket"].lower(), cpu_token, "asus", board_token,
    ))
...
def validate_h310_cpu_policy(platform: dict[str, Any], where: str) -> None:
    ...
    if board["product"] != "PRIME H310M-A R2.0":
        return
    platform_id = platform["id"]
    ...
    if not required_sources <= set(platform["source_refs"]):
        raise ValueError(f"{where}.source_refs 缺少 Intel/ASUS 官方组合证据")
