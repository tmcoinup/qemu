...
export VM_ROOT QEMU_IMG="$TMP_DIR/qemu-img" TRACE
...
[[ -f "$VM_ROOT/instances/vm4/disk.qcow2" ]] || fail "single-digit VM ID was not created"
...
grep -Eq "^create\\|$VM_ROOT/instances/vm4/\\.disk\\.qcow2\\.partial\\.[^|]+\\|12345$" "$TRACE" \
...
mkdir -p "$VM_ROOT/instances/vm21"
printf 'SSD_SIZE_BYTES=500107862016\n' \
    >"$VM_ROOT/instances/vm21/vm.conf"
...
[[ ! -e "$VM_ROOT/instances/vm21/disk.qcow2" ]] \
...
mkdir -p "$VM_ROOT/instances/vm22"
printf 'SSD_SIZE_BYTES=500107862016\n' \
    >"$VM_ROOT/instances/vm22/vm.conf"
"$CREATE_DISK" 22 --blank >"$TMP_DIR/profile-blank.out"
grep -Eq "^create\|$VM_ROOT/instances/vm22/\\.disk\\.qcow2\\.partial\\.[^|]+\\|500107862016$" \
...
[[ ! -e "$REQUIRED_VM_ROOT/instances/vm18/disk.qcow2" ]] || \
...
[[ ! -e "$VM_ROOT/instances/vm11/disk.qcow2" ]] \
...
mkdir -p "$DANGLING_VM_ROOT/bases"
ln -s missing-base.qcow2 "$DANGLING_VM_ROOT/bases/win10-base.qcow2"
VM_ROOT="$DANGLING_VM_ROOT" SIZE_BYTES=23456 "$CREATE_DISK" 16 --blank \
    >"$TMP_DIR/dangling-blank.out"
[[ -f "$DANGLING_VM_ROOT/instances/vm16/disk.qcow2" ]] || \
...
mkdir -p "$DATA_VM_ROOT/bases"
(
    cd "$DATA_VM_ROOT/bases"
...
[[ ! -e "$DATA_VM_ROOT/instances/vm15/disk.qcow2" ]] \
...
[[ ! -e "$VM_ROOT/instances/vm12/disk.qcow2" ]] \
    || fail "failed validation published a VM disk"
if find "$VM_ROOT/instances/vm12" -maxdepth 1 -name '*.partial.*' -print | grep -q .; then
    fail "failed validation left a partial disk"
fi

exec {DISK_HOLDER_FD}>"$VM_ROOT/run/vm14.disk.lock"
...
[[ ! -e "$VM_ROOT/instances/vm14/disk.qcow2" ]] \
