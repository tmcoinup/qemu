...
# 或 B350 等目标 PCH 的寄存器、BDF 或固件行为已经被完整模拟。
...
AUDITED_CANDIDATE_BINDINGS = {
    ...
    "normal-ryzen7-5800-ryzen3-1200-b350": (
        "supported", "asus-prime-b350-plus-ddr4",
    ),
}
# 同一 Guest 身份只允许为已实测的精确宿主增加一个受审计状态别名。该映射
# 防止维护者借“别名”任意复制 SKU、替换主板，或让两个宿主类共享正常层。
AUDITED_IDENTITY_ALIASES = {
    "normal-ryzen7-5800-ryzen3-1200-b350": "compat-zen-ryzen3-1200-b350",
}
ALLOWED_CANDIDATE_STATUS = frozenset({"supported", "compatibility"})

# 每个正常层都必须精确命中一组宿主类、一个 Guest 代际及一种内存类型。
# Ryzen 7 5800 不能借 Family 19h 的宽泛分类覆盖 5800X/5700X 等未实测型号。
SUPPORTED_POOL_BINDINGS = {
    frozenset({"e5-v3", "e5-v4"}): ("haswell", "DDR3"),
    frozenset({"amd-ryzen7-5800"}): ("zen", "DDR4"),
}

EXPECTED_SELECTION_POLICY: dict[str, Any] = {
    "compatibility_requires_explicit_allow": True,
    "supported_requires_explicit_allow": False,
    "host_supported_classes": ["e5-v3", "e5-v4", "amd-ryzen7-5800"],
    "priority": (
...
def validate_candidate_status(
    ...
    if status != "supported":
        return
    expected_platform = SUPPORTED_POOL_BINDINGS.get(frozenset(host_ids))
    if expected_platform != (guest_generation, memory_type):
        raise ValueError(
            f"{where} 未命中受审计宿主类对应的正常 Guest 代际/内存绑定"
        )
