...
def export_pairs(
    ...
    """保持 Linux/Windows 启动器共享的完整字段接口。"""
    cpu, board, memory = platform["cpu"], platform["board"], platform["memory"]
    devices, bios, system, tpm = (
        platform["devices"], platform["bios"], platform["system"], platform["tpm"],
    )
    smbios, igpu = cpu["smbios"], cpu["integrated_gpu"]
    chipset = devices["chipset"]
    root_port, xhci, nvme = devices["root_port"], devices["xhci"], devices["nvme"]
    nic, audio = devices["nic"], devices["audio"]
    return {
