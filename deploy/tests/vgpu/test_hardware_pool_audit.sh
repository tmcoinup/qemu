...
case "${G11_FAKE_MODE:-mixed}" in
    ...
    active-incompatible)
        if [[ "$cpu_spec" == *,enforce=on &&
              ( "$model" == Core-i7-3820 || "$model" == Core-i7-4820K ) ]]; then
            echo "qemu-system-x86_64: Host doesn't support requested features" >&2
            exit 1
        fi
        ;;
...
grep -Fx -- \
    'summary cpu=10 board=16 chipset_presentation=5 memory=37 combination=312 new_default=48 explicit_new=0 archived=261 legacy=3 ssd_512gb=10 optical=1 gpu_catalog=25 gpu_1gb=12 gpu_2gb=13 monitor_catalog=35 monitor_new=28' \
    <<<"$output" >/dev/null || fail 'hardware-pool summary/count contract changed'
grep -Fx -- \
    'brands board=3 memory=4 ssd=5 gpu_board=9 keyboard=3 relative_mouse=3 monitor_catalog=11 monitor_new=8' \
...
grep -Fx -- \
    'selection new_ready=48 explicit_ready=0 archived_existing=261 legacy_existing_ready=3 result=new-ready' \
    <<<"$output" >/dev/null || fail 'normal host selection audit changed'

[[ $(grep -c '^cpu_profile=' <<<"$output") == 10 ]] || \
    fail 'CPU audit row count changed'
[[ $(grep -c '^profile=' <<<"$output") == 312 ]] || \
