# qemu-9.2.0 兼容工具链（gmate 可选路径）

本目录记录并承载从 `qemu-9.2.0` 合入的 virtio-vga、硬件池、真机优化和部署
能力。`gmate` 的默认生产入口是上一层的 `deploy/start-vm.sh`，仍以 NVIDIA
mdev/vGPU 为主；这里的 `deploy/scripts/start-vm.sh` 是无 vGPU 环境和回归测试用
的兼容启动器。
