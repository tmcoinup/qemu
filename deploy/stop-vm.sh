#!/usr/bin/env bash
#
# stop-vm.sh — 关机 vm。
#
#   ./stop-vm.sh              优雅关机 (WinRM shutdown /s)
#   ./stop-vm.sh --force      guest 不响应时强杀：SIGTERM→SIGKILL→tmux kill→清 run/、 mdev
#
# 兼容 Ctrl+C：用户在 stop-vm.sh 等 QEMU 退出那几十秒里按 Ctrl+C，脚本会退出
# 但 VM 不会被意外杀掉 —— 再次 `./stop-vm.sh` 继续关即可。
#
set -euo pipefail
cd "$(dirname "$(readlink -f "$0")")"

VM_ID=${VM_ID:-1}
GUEST_IP_HINT=${GUEST_IP:-}
GUEST_USER=${GUEST_USER:-Administrator}
GUEST_PASS=${GUEST_PASS:-123456}
FORCE=0
SUDO_PW=${SUDO_PASSWORD:-123456}

while [[ $# -gt 0 ]]; do
    case "$1" in
        --force)     FORCE=1; shift ;;
        -h|--help)   sed -n '3,9p' "$0"; exit 0 ;;
        [0-9]*)      VM_ID="$1"; shift ;;       # bare number → vm id
        *) echo "unknown arg: $1"; exit 2 ;;
    esac
done

find_vm_ip() {
    local conf="${VM_ROOT:-/home/ubuntu/images/vms}/configs/vm${VM_ID}.conf"
    [[ -f "$conf" ]] || return 1
    # shellcheck source=/dev/null
    source "$conf"
    local mac_lc=${VM_MAC,,}
    ip -4 neigh show 2>/dev/null | awk -v m="$mac_lc" '$3=="br0" && tolower($5)==m && $1 ~ /^[0-9]/ {print $1; exit}'
}

cleanup_run() {
    rm -f run/vm${VM_ID}.pid run/vm${VM_ID}.qmp run/vm${VM_ID}.mon 2>/dev/null || true
    # Remove mdevs the killed QEMU left behind
    shopt -s nullglob
    local root=/sys/bus/mdev/devices e uuid
    for e in "$root"/*; do
        uuid=${e##*/}
        echo "[down] releasing mdev $uuid"
        echo "$SUDO_PW" | sudo -S -p '' sh -c "echo 1 > $e/remove" >/dev/null 2>&1 || true
    done
    shopt -u nullglob
}

if ! pgrep -f "qemu-system-x86_64.*-name vm${VM_ID}" >/dev/null; then
    echo "[down] no qemu-system for vm${VM_ID} — nothing to stop"
    cleanup_run
    exit 0
fi

# 决定走 graceful (WinRM) 还是 --force：
#   - --force 显式 → 强杀
#   - 没 --force 但 ARP 表查不到 guest IP → guest 没起到能联网阶段，
#     WinRM 必然不通 → 自动降级 --force（用户已经在按 stop-vm.sh 了，
#     不该让 ta 再加 flag 重跑）
if (( ! FORCE )); then
    GUEST_IP=${GUEST_IP_HINT:-$(find_vm_ip || true)}
    if [[ -z "$GUEST_IP" ]]; then
        echo "[down] guest IP 未知（ARP 表无条目，guest 还没起来），自动 --force"
        FORCE=1
    fi
fi

if (( FORCE )); then
    echo "[down] --force: TERM qemu-system-x86_64"
    pkill -TERM -f "qemu-system-x86_64.*-name vm${VM_ID}" || true
    for _ in $(seq 1 10); do
        pgrep -f "qemu-system-x86_64.*-name vm${VM_ID}" >/dev/null || break
        sleep 1
    done
    if pgrep -f "qemu-system-x86_64.*-name vm${VM_ID}" >/dev/null; then
        echo "[down] --force: KILL qemu-system-x86_64"
        pkill -KILL -f "qemu-system-x86_64.*-name vm${VM_ID}" || true
        sleep 2
    fi
    tmux kill-session -t "vm${VM_ID}" 2>/dev/null || true
    cleanup_run
else
    echo "[down] WinRM → ${GUEST_IP}: shutdown /s"
    python3 - <<PY 2>/dev/null || echo "[down] WinRM unreachable"
from pypsrp.client import Client
Client('${GUEST_IP}', username='${GUEST_USER}', password='${GUEST_PASS}', ssl=False, auth='ntlm') \
  .execute_ps('shutdown /s /t 3 /f /c "stop-vm.sh"')
PY
    echo -n "[down] waiting QEMU exit "
    for _ in $(seq 1 60); do
        if ! pgrep -f "qemu-system-x86_64.*-name vm${VM_ID}" >/dev/null; then
            echo " — done"
            break
        fi
        echo -n "."
        sleep 3
    done
    if pgrep -f "qemu-system-x86_64.*-name vm${VM_ID}" >/dev/null; then
        echo
        echo "[down] still running after 3 min — 再来一次带 --force"
        exit 1
    fi
    tmux kill-session -t "vm${VM_ID}" 2>/dev/null || true
    cleanup_run
fi
