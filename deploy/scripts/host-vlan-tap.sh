...
#   /usr/local/libexec/qemu-stealth-vlan-tap
#
# 安全模型：
#   1. 普通 VM 用户只能通过 sudoers 执行上面的 root:root 固定副本；
#   2. bridge、uplink 和允许的调用 UID/GID 只从 root 管理的配置读取；
#   3. 配置绝不 source/eval，只接受五个无引号、无空白、不可重复的固定字段；
#   4. 每个实例只能操作确定性名字 svtap<实例号>，cleanup-ifname 不能删除任意网卡；
...
#   cleanup-ifname <svtapN>          供 QEMU downscript/watchdog 幂等清理
#
...
readonly CONFIG_FILE="/etc/qemu/stealth-vlan.conf"
readonly STATE_DIR="/run/qemu-stealth-vlan"
readonly LOCK_FILE="/run/qemu-stealth-vlan.lock"
...
usage() {
    cat >&2 <<'EOF'
用法：
  host-vlan-tap check <instance> <vid>
  host-vlan-tap prepare <instance> <vid>
  host-vlan-tap cleanup-instance <instance>
  host-vlan-tap cleanup-ifname <svtapN>
EOF
    exit 2
}
...
# 任意合法业务 VID 第一次启动时在锁内自动加入物理 trunk；后续 VM 可直接复用。
...
cleanup_ifname_action() {
    ...
    if ! [[ "$tap" =~ ^svtap([0-9]{1,10})$ ]]; then
        die "cleanup-ifname 只接受 svtap<正整数>"
    fi
