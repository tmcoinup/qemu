...
//
// Enable MTRR early, set default type to write back.  With an mdev assigned,
// KVM honors guest MTRRs; leaving them disabled makes the main FV LZMA
// decompression run uncached and can add roughly 80 seconds to OVMF boot.
// Backported from upstream edk2 ce4c76e46d.
//
STATIC
VOID
SecMtrrSetup (
  ...
  )
{
  ...
  //
  // Initialize MTRR before decompressing the main firmware volume.
  //
  SecMtrrSetup ();
